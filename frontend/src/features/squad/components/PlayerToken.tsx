...
function PlayerPortrait({ compact, image }: { compact: boolean; image: string | null }) {
  ...
  return <div className={`relative mx-auto grid place-items-center overflow-hidden rounded-t-xl bg-[#f1eff2] ${compact ? 'h-[62px]' : 'h-[76px] tablet:h-[88px]'}`}>
    {!imageFailed && image && <img
      ...
      src={image}
    />}
    {imageFailed && <span aria-label="Generic player portrait" className="grid size-12 place-items-center rounded-full bg-[#ddd8df] text-[#766a78]" role="img"><Icon name="user" size={compact ? 28 : 34} /></span>}
  </div>
}

export function PlayerToken({ player, seasonName, compact = false, role }: Props) {
  ...
  return <div className={`group relative mx-auto w-full max-w-[112px] text-center ${compact ? 'max-w-[92px]' : ''}`}>
    ...
    <PlayerPortrait compact={compact} image={image} key={image ?? 'missing-kit'} />
