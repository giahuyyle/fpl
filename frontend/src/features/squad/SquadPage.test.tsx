...
describe('SquadPage', () => {
  ...
  it('shows 15 slots, searches filters, adds a player, and saves a draft', async () => {
    ...
    expect(within(marketDrawer).getByText('Total Pts')).toBeInTheDocument()
    expect(within(marketDrawer).getByText('£4.1m')).toBeInTheDocument()
    expect(within(marketDrawer).getByText('This season')).toBeInTheDocument()
    expect(within(marketDrawer).getByRole('columnheader', { name: 'Minutes' })).toBeInTheDocument()
