...
async function download(asset, fetchImpl, timeoutMs) {
  ...
  assertWebp(buffer, asset.sourceUrl)
  return buffer
}
...
export async function syncFplKits({
  ...
  for (const asset of assets) {
    ...
    if (!buffer) {
      buffer = await download(asset, fetchImpl, timeoutMs)
...
export function parseArguments(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    else if (['--season', '--bootstrap', '--output'].includes(argument)) {
